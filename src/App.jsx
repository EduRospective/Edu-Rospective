...
import { BookOpen, Calendar, FileText, Users, Bell, LogOut, PlusCircle, Download, Search, Settings, Export } from "lucide-react";

/*
 Edurospective — Teacher MVP
 - Single-file app (React) implementing core MVP:
   Dashboard, Templates (șabloane), Questionnaires, Calendar, Announcements, Backup/Export
 - Data persisted in localStorage for demo; can be wired to Firebase/Supabase later.
*/
...
const initialAnunturi = [
  { id: 1, titlu: "Ședință Consiliu Profesori", text: "Ședință miercuri, ora 16:00 - sala profesori.", data: new Date().toISOString() },
];
  ...
  return (
    <header className="flex items-center justify-between p-4 border-b">
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-lg bg-indigo-600 grid place-items-center text-white font-bold">ER</div>
        <div>
          <div className="font-semibold">Edurospective — Teacher MVP</div>
          <div className="text-sm text-zinc-600">Instrumente pentru calitate & management</div>
        </div>
      </div>
      <div className="flex items-center gap-3">
  ...
  return (
    <aside className="w-64 border-r p-4">
      <nav className="space-y-2">
        <button className={`block w-full text-left p-2 rounded ${view==='dashboard'?'bg-indigo-50':''}`} onClick={()=>setView('dashboard')}>Dashboard</button>
        <button className={`block w-full text-left p-2 rounded ${view==='templates'?'bg-indigo-50':''}`} onClick={()=>setView('templates')}><FileText className="inline mr-2"/>Șabloane PDI/POA</button>
        <button className={`block w-full text-left p-2 rounded ${view==='evaluari'?'bg-indigo-50':''}`} onClick={()=>setView('evaluari')}><BookOpen className="inline mr-2"/>Evaluări & Fișe</button>
        <button className={`block w-full text-left p-2 rounded ${view==='anunturi'?'bg-indigo-50':''}`} onClick={()=>setView('anunturi')}><Bell className="inline mr-2"/>Anunțuri</button>
        <button className={`block w-full text-left p-2 rounded ${view==='orar'?'bg-indigo-50':''}`} onClick={()=>setView('orar')}><Calendar className="inline mr-2"/>Calendar</button>
        <button className={`block w-full text-left p-2 rounded ${view==='directori'?'bg-indigo-50':''}`} onClick={()=>setView('directori')}><Users className="inline mr-2"/>Instrumente Director</button>
        <button className={`block w-full text-left p-2 rounded ${view==='setari'?'bg-indigo-50':''}`} onClick={()=>setView('setari')}><Settings className="inline mr-2"/>Setări & Backup</button>
      </nav>
    </aside>
  );
}
...
function TemplatesManager() {
  // For MVP we provide downloadable template links (as text blobs the user can copy)
  const templates = [
...
function EvaluariManager({ elevi }) {
  // Simple questionnaire builder + response collection stored locally
  ...
  function submitDummyResponse(formId) {
    // For demo: create fake responses from existing elevi
...
function AnunturiManager({ anunturi, setAnunturi }) {
  ...
  function exportAnnouncements(){ const rows = ['id,titlu,text,data'].concat(anunturi.map(a=>[a.id,`"${a.titlu}"`,`"${a.text}"`,a.data].join(','))).join('\\n'); const blob = new Blob([rows], { type: 'text/csv' }); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='anunturi.csv'; a.click(); URL.revokeObjectURL(url); }
  return (
    <div className="space-y-4">
      <div className="p-4 border rounded">
...
function OrarManager() {
  ...
  function exportCalendar(){ const rows = ['id,titlu,data'].concat(evenimente.map(e=>[e.id,`"${e.titlu}"`,e.data].join(','))).join('\\n'); const blob = new Blob([rows], { type: 'text/csv' }); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='calendar.csv'; a.click(); URL.revokeObjectURL(url); }
  return (
    <div className="space-y-4">
      <div className="p-4 border rounded">
        ...
        <div className="mt-4">
          ...
          {view==='setari' && (
            <div className="p-4 border rounded">
              ...
              <div className="mt-4">
                <button className="px-3 py-1 border rounded text-sm" onClick={()=>{ alert('Backup descărcat ca JSON'); const data = { cursuri, elevi, anunturi, teme }; const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'backup_edurospective.json'; a.click(); URL.revokeObjectURL(url); }}><Download className="inline mr-2"/>Descarcă backup</button>
              </div>
            </div>
          )}
        </div>
